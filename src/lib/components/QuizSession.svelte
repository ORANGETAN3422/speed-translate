...
<div class="fixed inset-0 flex flex-col bg-white">
    ...
    {#if finished}
        <main class="flex flex-1 flex-col items-center gap-6 overflow-y-auto px-4 py-8">
            ...
            <button class="rounded border px-4 py-2" onclick={onclose} autofocus>
                Return to menu
            </button>
        </main>
    {:else}
        <main class="flex flex-1 flex-col items-center justify-center gap-6 px-4">
            <div class="flex w-full max-w-md items-center gap-3">
                ...
                <span class="text-sm text-gray-500 tabular-nums">{progress} / {GOAL}</span>
            </div>

            <div class="text-center">
                <p class="text-2xl">{currentSentence?.jp}</p>
                {#if revealed}
                    <p class="mt-2 text-gray-500">{currentSentence?.en}</p>
                {/if}
            </div>
